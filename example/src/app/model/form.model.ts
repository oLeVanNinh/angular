      ...
      for (let errorName in this.errors) {
        switch (errorName) {
          ...
          case "maxlength":
            ...
            break;
          case "limit":
            messages.push(`A ${this.label} cannot be more thant ${this.errors['limit'].limit}`);
            break;
          case "pattern":
...
export class ProductFormGroup extends FormGroup {
  ...
  getFormValidationMessages(form: any): string[] {
    ...
    this.productControls.forEach(c => c.getValidationMessages().forEach(m => messsages.push(m)));
    return messsages;
  }
}
